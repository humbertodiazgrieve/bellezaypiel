...
BASE_DIR = r"C:\Users\User\proyectos ia\bellezaypiel\bellezaypiel-web\src\pages"

# Estructura de URLs del usuario
structure = {
    "yoga-facial": [
        "que-es-yoga-facial",
        "beneficios-yoga-facial",
        "yoga-facial-para-principiantes",
        "yoga-facial-antes-y-despues",
        "errores-comunes-yoga-facial",
        "contraindicaciones-yoga-facial",
    ],
    "ejercicios-faciales": [
  ...
  <div style="background:var(--color-bg);min-height:100vh;">
    ...
    <div style="position:relative;padding:52px 56px 0;overflow:hidden;max-width:1280px;margin:0 auto;">
      ...
      <div style="position:relative;display:flex;gap:8px;font-size:13.5px;color:var(--color-neutral-700);margin-bottom:20px;">
        <a href="/" style="color:var(--color-neutral-700);">Inicio</a><span>/</span><a href="/{section}/" style="color:var(--color-neutral-700);">{section_label}</a><span>/</span><span style="font-weight:600;color:var(--color-text);">{title}</span>
      </div>
...
def create_pages():
    for section, pages in structure.items():
        ...
        for page in pages:
            file_path = os.path.join(section_dir, f"{page}.astro")
            title = title_from_slug(page)
            ...
            with open(file_path, "w", encoding="utf-8") as f:
                f.write(content)
            print(f"Created: {section}/{page}.astro")
